...
def morphological_embedding(Affinity_df_trs_total, soma_pos, annotation, color_neuron_dict, max_proj_labels,
    ...
    Affinity_mat_red = umap.UMAP(n_components=2, metric='precomputed', random_state=42).fit_transform(Affinity_mat_trs)
    ac = AgglomerativeClustering(n_clusters = n_clust)
    mdist_clust_labels = ac.fit_predict(Affinity_mat_red)

    # Use two anchor points for gradient indexing
    # Anchor 1: point furthest from centroid, Anchor 2: point furthest from Anchor 1
    centroid = np.mean(Affinity_mat_red, axis=0)
    root_pt1 = np.argmax([euclidean(p, centroid) for p in Affinity_mat_red])
    root_pt2 = np.argmax([euclidean(p, Affinity_mat_red[root_pt1, :]) for p in Affinity_mat_red])

    diff_grad_root1 = np.array([euclidean(Affinity_mat_red[root_pt1, :], p) for p in Affinity_mat_red])
    diff_grad_root2 = np.array([euclidean(Affinity_mat_red[root_pt2, :], p) for p in Affinity_mat_red])

    # Relative index: d1 / (d1 + d2)
    diff_grad_nrm = diff_grad_root1 / (diff_grad_root1 + diff_grad_root2)
    ...
    return Affinity_mat_red, diff_grad_root1, diff_grad_nrm, neuronid_to_grad, mdist_clust_labels
